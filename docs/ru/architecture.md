...
- Плейбуки выполняются на этом хосте, пока не включена изоляция, — а тогда в
  контейнере, который уже должен быть на машине. Образы здесь не собираются и не
  скачиваются (ADR-0008), а в контейнерном варианте режим недоступен: дотянуться
  до рантайма означало бы отдать его сокет.
